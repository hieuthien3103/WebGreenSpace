...
if (empty($GLOBALS['mvc_template_rendering'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        (new AdminToolController())->fixImagesForm()->send();
    }
    return;
}
...
<div class="rounded-[1.75rem] border border-[#d9e9de] bg-white p-6 shadow-sm">
    <?php if ($results === null): ?>
        <div class="mb-5">
            ...
            <p class="mt-2 text-sm text-[#6e8d7b]">Thao tác này sẽ chuẩn hóa tất cả đường dẫn ảnh sản phẩm trong database.</p>
        </div>
        <form method="POST" action="fix_images.php">
            <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
            <button type="submit" class="inline-flex items-center rounded-full bg-[#2e9b63] px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#22784d]">Chuẩn hóa ngay</button>
        </form>
    <?php else: ?>
