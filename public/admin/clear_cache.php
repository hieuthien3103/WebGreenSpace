...
if (empty($GLOBALS['mvc_template_rendering'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        (new AdminToolController())->clearCacheForm()->send();
    }
    return;
}
...
<div class="space-y-6">
    <?php if ($results === null): ?>
        <div class="rounded-[1.75rem] border border-[#d9e9de] bg-white p-6 shadow-sm">
            <h2 class="text-2xl font-extrabold text-[#102118]">Clear Cache</h2>
            <p class="mt-2 text-sm text-[#6e8d7b]">Thao tác này sẽ xóa OPcache và file stat cache trên server.</p>
            <form method="POST" action="clear_cache.php" class="mt-4">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                <button type="submit" class="inline-flex items-center rounded-full bg-[#2e9b63] px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#22784d]">Xóa cache ngay</button>
            </form>
        </div>
    <?php else: ?>
