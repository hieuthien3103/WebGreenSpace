...
\begin{titlepage}
    ...
    {\huge \textbf{BÁO CÁO ĐỒ ÁN WEB}} \\[1cm]
    {\Large \textbf{XÂY DỰNG WEBSITE BÁN CÂY CẢNH GREENSPACE}}

    \vfill
    \hfill
    \begin{minipage}{0.75\textwidth}
        \large
        \textbf{Giáo viên hướng dẫn:} Nguyễn Thanh Sang \\[0.5cm]
        \textbf{Nhóm sinh viên thực hiện:} \\[0.2cm]
        \begin{tabular}{@{}l@{\hspace{0.5cm}}l@{}}
            Nguyễn Phạm Hiếu Thiện & MSSV: 3123410353 \\
            Phan Trọng Tiến        & MSSV: 3123410376 \\
        \end{tabular}
    \end{minipage}
...
\subsection{Thực trạng và nhu cầu}
...
Về phía người dùng cuối (Khách hàng), hệ thống cần cung cấp giao diện trực quan và thân thiện, cho phép tìm kiếm và lọc sản phẩm theo nhiều tiêu chí đặc thù của cây cảnh như mức độ chăm sóc, nhu cầu ánh sáng và môi trường phù hợp. Quy trình đặt hàng phải được thiết kế tối giản, mượt mà, với giỏ hàng AJAX không tải lại trang và hệ thống sổ địa chỉ giao hàng thông minh. Đặc biệt, mỗi sản phẩm cần đi kèm thông tin hướng dẫn chăm sóc chi tiết để hỗ trợ người mua, kể cả những người chưa có kinh nghiệm trồng cây.
...
\subsection{Khảo sát và phân tích đối thủ cạnh tranh}
...
\begin{table}[H]
    \centering
    \caption{Bảng so sánh tính năng giữa GreenSpace và các đối thủ cạnh tranh}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{4cm}
        ...
        \hline
        \textbf{Tiêu chí} & \textbf{Web A (Cây Xanh Online)} & \textbf{Web B (Vườn Nhà Xanh)} & \textbf{Web C (Shop Cây Cảnh VN)} & \textbf{GreenSpace} \\ \hline
        ...
        Đánh giá sản phẩm & Không & Có (đơn giản) & Không & Có (1--5 sao + bình luận) \\ \hline
...
\textbf{Web A --- Cây Xanh Online:} Đây là một trong những website bán cây cảnh lâu đời hơn trên thị trường với danh mục sản phẩm tương đối đa dạng. Điểm mạnh của Web A là giao diện quen thuộc, dễ nhận biết với người dùng trung tuổi, và hệ thống phân loại danh mục cây khá rõ ràng. Tuy nhiên, website này bộc lộ nhiều hạn chế nghiêm trọng về mặt kỹ thuật: thời gian tải trang kéo dài từ 3 đến 5 giây do không tối ưu ảnh sản phẩm, toàn bộ quy trình giỏ hàng sử dụng form submit truyền thống buộc trang phải tải lại sau mỗi thao tác, và không có tính năng sổ địa chỉ giao hàng khiến người dùng phải nhập lại thông tin giao hàng cho mỗi đơn hàng. Thông tin về cách chăm sóc cây rất sơ lược, chủ yếu là một đoạn văn ngắn mà không có bảng thông số chuẩn hóa.

\textbf{Web B --- Vườn Nhà Xanh:} Website này hướng đến phân khúc khách hàng yêu thích vườn rau và cây ăn quả tại nhà, với thiết kế mang phong cách thủ công (handmade). Điểm nổi bật là có tính năng đánh giá sản phẩm từ người mua thực, tạo được một mức độ tin cậy nhất định. Tuy nhiên, Web B có tốc độ tải trang rất chậm (5--8 giây), giao diện responsive chưa được tối ưu tốt trên thiết bị di động --- màn hình điện thoại hiển thị bị vỡ bố cục ở một số trang --- và quy trình thanh toán phức tạp, nhiều bước không cần thiết. Hệ thống backend quản trị đơn giản, thiếu các tính năng theo dõi tồn kho và lịch sử biến động kho.

\textbf{Web C --- Shop Cây Cảnh VN:} Đây là đối thủ có tính năng tương đối hoàn thiện nhất trong ba website được khảo sát, với hỗ trợ thanh toán chuyển khoản ngân hàng, tính năng coupon cơ bản và giao diện responsive chấp nhận được. Điểm đáng chú ý là Web C sử dụng AJAX cho một phần của quy trình giỏ hàng (thêm sản phẩm), nhưng các bước cập nhật số lượng và xóa sản phẩm vẫn dùng form submit truyền thống. Tuy nhiên, website này vẫn thiếu sổ địa chỉ giao hàng, không có thông tin chăm sóc cây chi tiết và không có hệ thống quản lý kho tự động.
...
\subsection{Phạm vi nghiên cứu và giới hạn của đề tài}
...
\begin{center}
\textit{Hình 2.1: Sơ đồ luồng xử lý trong mô hình MVC cơ bản}
\end{center}
...
\begin{table}[H]
    \centering
    \caption{So sánh kiến trúc MVC thuần và MVC + Service Layer}
    \vspace{0.2cm}
...
\begin{center}
\textit{Hình 2.2: Sơ đồ kiến trúc MVC + Service Layer trong hệ thống GreenSpace}\\
\textit{Client $\rightarrow$ Router $\rightarrow$ Controller $\rightarrow$ Service $\rightarrow$ Model $\rightarrow$ Database}
\end{center}
...
\begin{table}[H]
    \centering
    \caption{So sánh Docker Container và Máy ảo (Virtual Machine)}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|Y|}
        \hline
...
\begin{center}
\textit{Hình 2.3: Kiến trúc Docker của hệ thống GreenSpace với 3 container}
\end{center}
...
\begin{table}[H]
    \centering
    \caption{Yêu cầu phi chức năng của hệ thống GreenSpace}
    \vspace{0.2cm}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ Use Case của tác nhân Khách hàng}
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-02 --- Đăng nhập tài khoản}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-03 --- Thêm sản phẩm vào giỏ hàng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-04 --- Thanh toán đặt hàng}
    \vspace{0.2cm}
    \small
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-05 --- Quản lý sản phẩm}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-06 --- Quản lý tồn kho}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-07 --- Theo dõi trạng thái đơn hàng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-08 --- Quản lý người dùng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-09 --- Quản lý mã giảm giá}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Đặc tả Use Case UC-10 --- Cập nhật thông tin cá nhân}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|Y|}
        \hline
        \textbf{Thuộc tính} & \textbf{Nội dung} \\ \hline
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Controller:} Tầng trung gian, tiếp nhận request, gọi Service/Model và trả về View tương ứng.
    ...
    \item \textbf{Router:} Lớp định tuyến tùy chỉnh, ánh xạ Friendly URL vào Controller/Method qua biểu thức chính quy.
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Các công nghệ chính được sử dụng trong hệ thống GreenSpace}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{2.5cm}
        ...
        PHP & 8.2 & Ngôn ngữ Back-end chính; hỗ trợ PDO mạnh mẽ, dễ triển khai \\ \hline
        MySQL & 8.0 & CSDL quan hệ; phù hợp mô hình có nhiều khóa ngoại, hỗ trợ Transaction \\ \hline
        HTML5/CSS3 & --- & Xây dựng giao diện tầng View \\ \hline
        JavaScript & --- & Xử lý client-side: giỏ hàng AJAX, xác thực form, toast thông báo \\ \hline
        Docker & --- & Container hóa môi trường, đảm bảo nhất quán dev/production \\ \hline
        Apache & --- & Web server; mod\_rewrite hỗ trợ Friendly URLs \\ \hline
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        \item \texttt{/core}: Router tùy chỉnh xử lý điều hướng URL.
        \item \texttt{/controllers}: Tiếp nhận HTTP request, điều phối luồng dữ liệu.
        \item \texttt{/services}: \texttt{CheckoutService}, \texttt{CartService}, \texttt{ProductService}.
        \item \texttt{/models}: Tương tác Database qua PDO.
        \item \texttt{/views}: Template PHP/HTML theo module.
    \end{itemize}
    \item \textbf{/public} --- Điểm vào duy nhất (\texttt{index.php}), tài nguyên tĩnh CSS/JS/Images.
    \item \textbf{/config} --- Cấu hình hệ thống và kết nối CSDL.
    \item \textbf{/docker} --- \texttt{Dockerfile} và \texttt{docker-compose.yml}.
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{Ví dụ một số route tiêu biểu trong hệ thống}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{4cm}
        ...
        \texttt{/san-pham/\{slug\}} & \texttt{ProductController::detail} & Chi tiết sản phẩm \\ \hline
        \texttt{/danh-muc/\{slug\}} & \texttt{CategoryController::index} & Danh sách theo danh mục \\ \hline
        \texttt{/gio-hang} & \texttt{CartController::index} & Trang giỏ hàng \\ \hline
        \texttt{/dat-hang} & \texttt{CheckoutController::index} & Trang thanh toán \\ \hline
        \texttt{/admin/don-hang} & \texttt{Admin/OrderController::index} & Quản lý đơn hàng (admin) \\ \hline
    \end{tabularx}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Biểu đồ Thực thể Liên kết (ERD) chi tiết hệ thống GreenSpace}
    \label{fig:erd_full}
\end{figure}
...
\begin{table}[H]
    \centering
    \caption{Bảng 1: Đặc tả bảng \texttt{users} --- Quản lý tài khoản}
    \vspace{0.2cm}
...
\begin{table}[H]
    \centering
    \caption{Bảng 2: Đặc tả bảng \texttt{addresses} --- Sổ địa chỉ giao hàng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 3: Đặc tả bảng \texttt{categories} --- Phân loại danh mục}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 4: Đặc tả bảng \texttt{products} --- Hồ sơ cây cảnh}
    \vspace{0.2cm}
...
\begin{table}[H]
    \centering
    \caption{Bảng 5: Đặc tả bảng \texttt{product\_images} --- Thư viện ảnh phụ}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 6: Đặc tả bảng \texttt{product\_variants} --- Biến thể sản phẩm}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 7: Đặc tả bảng \texttt{cart} --- Giỏ hàng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 8: Đặc tả bảng \texttt{orders} --- Phiếu đặt hàng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 9: Đặc tả bảng \texttt{order\_details} --- Chi tiết mặt hàng trong đơn}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 10: Đặc tả bảng \texttt{payments} --- Giao dịch thanh toán}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 11: Đặc tả bảng \texttt{coupons} --- Mã giảm giá}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3.5cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 12: Đặc tả bảng \texttt{coupon\_usages} --- Lịch sử dùng mã}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 13: Đặc tả bảng \texttt{reviews} --- Đánh giá của khách hàng}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 14: Đặc tả bảng \texttt{wishlists} --- Danh mục yêu thích}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 15: Đặc tả bảng \texttt{inventory\_logs} --- Nhật ký biến động kho}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
\begin{table}[H]
    \centering
    \caption{Bảng 16: Đặc tả bảng \texttt{admin\_notifications} --- Thông báo hệ thống (Bổ sung mở rộng)}
    \vspace{0.2cm}
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}p{3cm}|l|l|Y|}
        \hline
        \textbf{Tên trường} & \textbf{Kiểu dữ liệu} & \textbf{Ràng buộc} & \textbf{Mô tả} \\ \hline
...
Trong phiên bản repo được kiểm tra ngày \textbf{09/04/2026}, thư mục \texttt{tests/}
chưa có tệp \texttt{test\_cart.php}. Vì vậy nếu viết báo cáo theo đúng hiện trạng
dự án, không nên liệt kê lệnh \texttt{php tests/test\_cart.php} khi chưa bổ sung thêm
file kiểm thử tương ứng. Kết quả kiểm thử được trình bày chi tiết trong
Mục~\ref{sec:ketqua-kiemthu}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\linewidth}{|>{\centering\arraybackslash}p{1cm}
        ...
        11 & Docker hóa             & Hoàn thiện & 3 service, môi trường nhất quán \\ \hline
    \end{tabularx}
\end{table}
...
thành một nền tảng thương mại điện tử chuyên nghiệp. Dưới đây là các định
hướng phát triển cụ thể được nhóm đánh giá là có tính khả thi cao và mang
lại giá trị thực tiễn lớn nhất:

\begin{itemize}[leftmargin=*]
    \item \textbf{Tích hợp cổng thanh toán và đơn vị vận chuyển:}
    Triển khai SDK để kết nối API trực tiếp với VNPay hoặc MoMo xử lý dòng
    tiền thực tế. Song song đó, tích hợp API của Giao Hàng Tiết Kiệm (GHTK)
    hoặc Giao Hàng Nhanh (GHN) để tự động tạo vận đơn, tính phí ship theo
    địa chỉ và cho phép khách hàng theo dõi hành trình bưu kiện ngay trên
    website.

    \item \textbf{Hệ thống thông báo đa kênh:}
    Xây dựng module Email Transactional qua SendGrid hoặc Mailgun để gửi
    xác nhận đặt hàng, cập nhật trạng thái vận chuyển và thông báo khuyến
    mãi. Bổ sung kênh Zalo ZNS (Zalo Notification Service) --- kênh thông
    báo được người dùng Việt Nam ưa chuộng hơn so với SMS truyền thống ---
    để tăng tỷ lệ khách hàng nhận được thông tin kịp thời.
    ...
    phù hợp với phong thủy, sở thích và điều kiện không gian sống của từng
    khách hàng.
    ...
    dụng kỹ thuật \textbf{Lazy Loading} cho ảnh sản phẩm và phân trang
    vô hạn (Infinite Scroll) cho trang danh sách để cải thiện trải nghiệm
    người dùng trên kết nối mạng chậm.
    ...
    API (JSON). Đây là bước mở rộng tự nhiên khi tỷ lệ người dùng mua sắm
    qua điện thoại di động tại Việt Nam ngày càng tăng cao.
\end{itemize}
...
\noindent\textbf{Tên đề tài:} Xây dựng website bán cây cảnh GreenSpace
...
\begin{tabular}{@{}l@{\hspace{1cm}}l@{}}
    Nguyễn Phạm Hiếu Thiện & MSSV: 3123410353 \\[0.2cm]
    Phan Trọng Tiến         & MSSV: 3123410376 \\
\end{tabular}
...
% --- Dòng kẻ để giảng viên ghi nhận xét ---
\noindent\rule{\linewidth}{0.4pt}\\[0.65cm]
\noindent\rule{\linewidth}{0.4pt}\\[0.65cm]
\noindent\rule{\linewidth}{0.4pt}\\[0.65cm]
\noindent\rule{\linewidth}{0.4pt}\\[0.65cm]
\noindent\rule{\linewidth}{0.4pt}\\[0.65cm]
...
\noindent\textbf{Điểm số:}

\vspace{0.4cm}

\begin{tabular}{|>{\centering\arraybackslash}p{5cm}|>{\centering\arraybackslash}p{4cm}|>{\centering\arraybackslash}p{4cm}|}
    \hline
    \textbf{Tiêu chí} & \textbf{Điểm tối đa} & \textbf{Điểm đạt được} \\
    \hline
    Nội dung báo cáo & 40 & \\[0.4cm]
    \hline
    Kết quả thực hiện & 40 & \\[0.4cm]
    \hline
    Hình thức trình bày & 20 & \\[0.4cm]
    \hline
    \textbf{Tổng cộng} & \textbf{100} & \\[0.4cm]
    \hline
\end{tabular}
...
\begin{flushright}
    \textit{Thành phố Hồ Chí Minh, ngày \underline{\hspace{0.8cm}} tháng \underline{\hspace{0.8cm}} năm 2026}\\[1.5cm]
    \textbf{Giảng viên hướng dẫn}\\[0.3cm]
    \textit{(Ký và ghi rõ họ tên)}\\[2.5cm]
    \textbf{Nguyễn Thanh Sang}
\end{flushright}
\end{document}