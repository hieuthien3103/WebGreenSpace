...
<div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <nav class="mb-8 text-sm">
        <ol class="flex items-center gap-2 text-text-secondary dark:text-gray-400">
            <li><a class="hover:text-primary transition-colors" href="<?= base_url() ?>">Trang chủ</a></li>
            <li class="material-symbols-outlined text-[16px]">chevron_right</li>
            <li><a class="hover:text-primary transition-colors" href="<?= base_url('products') ?>">Cửa hàng</a></li>
            <li class="material-symbols-outlined text-[16px]">chevron_right</li>
            <?php if (!empty($product['category_name']) && !empty($product['category_slug'])): ?>
            <li><a class="hover:text-primary transition-colors" href="<?= base_url('category/' . $product['category_slug']) ?>"><?= clean($product['category_name']) ?></a></li>
            <li class="material-symbols-outlined text-[16px]">chevron_right</li>
            <?php endif; ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        <div class="flex flex-col gap-4">
            ...
            <div class="grid grid-cols-4 gap-3">
                ...
                <button type="button" class="aspect-square rounded-lg overflow-hidden border-2 <?= $index === 0 ? 'border-primary' : 'border-transparent hover:border-primary transition-colors' ?>" onclick="changeImage(this)">
        ...
        <div class="flex flex-col">
            <div class="mb-6">
                ...
                <div class="flex items-baseline gap-4 mb-6">
                    ...
                    <span class="px-3 py-1 bg-red-500 text-white text-sm font-bold rounded-lg">-<?= (int)($product['discount_percentage'] ?? 0) ?>%</span>
                    <?php endif; ?>
                </div>
            ...
            <div class="mb-6 flex flex-wrap gap-2">
                <span class="px-4 py-2 rounded-full text-xs font-semibold bg-white dark:bg-[#1e2b24] border border-[#e9f2ec] dark:border-gray-700 text-text-secondary flex items-center gap-1">
                    <span class="material-symbols-outlined text-[16px]">spa</span> Chăm sóc: <?= clean($product['temp_care'] ?? $product['care_level'] ?? 'Tiêu chuẩn') ?>
                </span>
                <span class="px-4 py-2 rounded-full text-xs font-semibold bg-white dark:bg-[#1e2b24] border border-[#e9f2ec] dark:border-gray-700 text-text-secondary flex items-center gap-1">
                    <span class="material-symbols-outlined text-[16px]">wb_sunny</span> Ánh sáng: <?= clean($product['light_care'] ?? $product['light_requirement'] ?? 'Trung bình') ?>
                </span>
                <span class="px-4 py-2 rounded-full text-xs font-semibold bg-white dark:bg-[#1e2b24] border border-[#e9f2ec] dark:border-gray-700 text-text-secondary flex items-center gap-1">
                    <span class="material-symbols-outlined text-[16px]">water_drop</span> Nước: <?= clean($product['water_care'] ?? $product['water_requirement'] ?? 'Trung bình') ?>
                </span>
            </div>

            <form class="mb-6 space-y-4" action="<?= base_url('cart') ?>" method="POST" data-product-image="<?= clean($product['image_url'] ?? image_url('products/default.jpg')) ?>">
                <input type="hidden" name="csrf_token" value="<?= clean(csrf_token()) ?>">
                <input type="hidden" name="action" value="add">
                <input type="hidden" name="product_id" value="<?= (int)$product['id'] ?>">
                <input type="hidden" name="redirect_to" value="<?= clean('product/' . $product['slug']) ?>">
    ...
    <section>
        <div class="flex items-center justify-between mb-8">
            ...
            <a class="text-primary font-bold flex items-center gap-1 hover:gap-2 transition-all" href="<?= base_url('products') ?>">
                Xem tất cả
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <article class="group bg-white dark:bg-[#1e2b24] rounded-2xl overflow-hidden border border-transparent hover:border-[#e9f2ec] dark:hover:border-gray-700 hover:shadow-xl transition-all duration-300 flex flex-col">
                <a href="<?= base_url('product/' . $relProduct['slug']) ?>" class="relative w-full aspect-[4/5] overflow-hidden bg-gray-100 dark:bg-gray-800">
                ...
                <div class="p-5 flex flex-col flex-1">
                    ...
                    <div class="mt-auto flex items-center justify-between">
                        ...
                        <a href="<?= base_url('product/' . $relProduct['slug']) ?>" class="size-10 rounded-full bg-primary text-white flex items-center justify-center shadow-lg shadow-primary/30 hover:bg-primary-dark active:scale-95 transition-all">
